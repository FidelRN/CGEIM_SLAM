...
// Object to draw
vector<glm::vec3> vert_aux;
vector<glm::vec2> uvs_aux;
vector<glm::vec3> normals_aux; // Won't be used at the moment.
...
glClearColor(0,0,0,0);
        ...
        for (size_t i = 0; i < vert_aux.size(); i += 1)
        {
            a = vert_aux[i];
            b = uvs_aux[i];
...
void ViewerAR::scale3DModel(float scaleFactor)
{
    for (size_t i = 0; i < vert_aux.size(); i += 1)
    {
        vert_aux[i] = vert_aux[i] * vec3(scaleFactor * 1.0f, scaleFactor * 1.0f, scaleFactor * 1.0f);
    }

    for (size_t i = 0; i < normals_aux.size(); i += 1)
    {
        normals_aux[i] = normals_aux[i] * vec3(scaleFactor * 1.0f, scaleFactor * 1.0f, scaleFactor * 1.0f);
    }

    for (size_t i = 0; i < uvs_aux.size(); i += 1)
    {
        uvs_aux[i] = uvs_aux[i] * vec2(scaleFactor * 1.0f, scaleFactor * 1.0f);
    }
}
